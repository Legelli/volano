...
Con l'approssimazione $\Delta h / h = \SI{0.063 \pm 0.003}{}$\footnote{La media è stata inferita dall'istogramma, associandole una incertezza pari alla deviazione standard sulla radice del numero di dati presi.}, dove l'incertezza è la deviazione standard della media, vale la relazione	
\begin{equation}
...
Come valore del momento di attrito frenante $M_a$ abbiamo utilizzato quello precedentemente stimato nella sezione~\ref{subsec:seconda}, in quanto è maggiormente preciso. Riteniamo che eventuali discrepanze della nostra stima dal \emph{valore vero}\footnote{È qui doveroso enfatizzare questo termine in quanto l'attrito con l'aria, essendo dovuto al moto casuale delle particelle che compongono il mezzo, non può essere esplicitato con una rigorosa trattazione meccanica, pertanto il coefficiente ha solo valore indicativo della influenza sul moto del corpo.} siano dovute alla eventuale non dipendenza lineare del momento d'attrito con l'aria dalla velocità. L'incertezza, dell'ordine dello $0.5\%$ è dovuta alle misure del raggio del dischetto attorno a cui si avvolge il filo e alla precedente stima del momento d'attrito frenante, oltre all'errore statistico sulla media della velocità limite poiché, come si nota dal grafico~\ref{fig:vel_attrito_aria}, la velocità oscilla in modo consistente attorno a $v_\text{lim}$.
...
int main() {
  ...
  while (!feof(pfr)) { //Continua finche' il file non e' finito
    ...
    if (b == 0 && h < hTemp) { //Caso di un massimo
      ...
      hMin = h;
      	if((n%2)==1) { //Prende un valore sì e uno no
